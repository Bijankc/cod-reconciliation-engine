-- ===========================================================================
-- FROZEN as of 2026-08-22, end of Phase 4. DO NOT EDIT THIS FILE.
--
-- The next schema change of any size is 0002_*.sql. Not "once we deploy" — NOW,
-- unconditionally, with no remote database in existence.
--
-- The original rule froze this file the moment `d1 migrations apply --remote`
-- first succeeded, which was correct but read as a licence: three in-place edits
-- happened under it (Decision 3's discrepancy_reason and orphan_events, then the
-- 'received' outcome, then Decision 9's delivery_count). A rule whose only
-- enforcement is a future event that has not happened yet is not enforcing
-- anything. Three rewrites is the signal to stop.
--
-- Writing 0002 against a local database costs exactly what editing this file
-- costs, and it is the only version of the habit that survives contact with a
-- real deployment. So the habit starts now, while it is free.
--
-- Why the schema is considered settled: `orders` carries the full six-value
-- ladder plus its discrepancy_reason invariant; `order_events` carries the
-- ledger's three verdicts plus delivery_count; `orphan_events` is terminal by
-- design. Phase 5 is read-only. The one change that is genuinely FORESEEABLE is
-- a dead-letter landing table in Phase 6, if poison events get recorded rather
-- than only observed — and that is a new table, which is a textbook 0002.
-- ===========================================================================
